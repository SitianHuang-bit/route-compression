...
def mergeData_ipint(*datas)->list:#*datas表示这个函数接受一个不确定数量的参数，
    """合并传入的 data
    ...
    local = set()  #位置s
    for x in datas:
        local.add((x.lng, x.lat))

    list1 = list(local)#集合转换成列表
    local2 = mergeLocal(list1)  #合并位置   返回值是元组
    #判断合并后的经纬度是否与它们相聚在规定的范围内
    for i in range(len(list1)):
         if distance(local2[0], local2[1], list1[i][0], list1[i][1]) > 1000:
    ...
    for v in datas:
        a.append(v.ipIndex)
        a.append(v.ipEnd)
    data1 = data(datas[0].iprefix, datas[0].asn, local2[0], local2[1], min(a),
                max(a))
