...
from compress.zip.lib2.binarySearch import binarySearchBig, binarySearchSmall
...
def add_dele(merged_data:list[data],querylist:list[dataInt],querydict:dict,datas_dict:dict,interval_new:list):
    ...
    index_pre = binarySearchSmall(querylist,value_pre[0],value_pre[1],interval_new[0])#找新区间在querylist里的下标
    index_last = binarySearchBig(querylist,value_last[0],value_last[1],interval_new[1])
    ...
    if merged_data[0].ipEnd > merged_data[1].ipEnd:
        ...
        index_last2 = binarySearchBig(querylist,tmp2[0],tmp2[1],merged_data[1].ipEnd)#找到第二个数据的ipEnd在querylist的下标
    ...
    #循环遍历 querylist 中从 index_pre 到 index_last 的每个元素。这个范围内的 querylist 元素对应于合并操作影响的区间。
    ...
    # 第一部分 [[index_pre, index_last]] 表示受影响的 querylist 范围。
    # 第二部分 [[merged_data[0], del_dict_Index[0]], [merged_data[1], del_dict_Index[1]]] 包含了被合并的 data 对象及其在 querylist 中对应的原始索引。
...
def deal_datas_diss(datas:list,diss:list,minIndex:int,range1):
    ...
    diss.pop(minIndex)
    # diss 更新
    if minIndex > 0:#更新合并后的前缀区间和它的前一个的前缀区间的距离
        diss[minIndex - 1] = distance_data(datas[minIndex - 1], datas[minIndex])
    if minIndex + 1 < len(datas):##更新合并后的前缀区间和它的后一个的前缀区间的距离
...
def com_algorithm(initial_datas_dict:dict,initial_querylist:list[dataInt],datas:list[data],diss:list,datas_dict:dict,querylist:list[dataInt],querydict:dict,range1:float,result_file:str)->list[data]:
    ...
    while len(datas) > 1:
        # if (len(datas) == 23510):
        #     saveAllData(datas,'G:/前缀压缩/BGP_ipv6/v_bgps/data/final.db')
        #     saveAllData(datas,'G:/前缀压缩/BGP_ipv6/v_bgps/data/final')
        #     return datas
        ...
        #用于存放前后查询的经纬度
        new_data_list = [] #存放合并后出现的ci和对应的经纬度
        min_diss = min(diss)
        flag = True #判断是否由于合并后区间大小相同且有重叠问题造成的不能合并
        if min_diss <= 2*range1:
            minIndex = getMinIndex(diss,min_diss)#获取diss中最小值的下标
            
            com_data = mergeData_ipint(datas[minIndex], datas[minIndex + 1])#返回的是一个list类型，只有两个元素，第一个元素是data数据类型，第二个表示能否合并
            if com_data[1]==True : #根据聚合后经纬度到原始经纬度的距离判断能否合并      com_data[1]有True和False两种值
                ...
                #合并后处理dict
                datas_dict[str(len(datas_dict)+1)] = com_data[0] #将新合并的data添加到datas_dict里
                
                #合并后处理querylist  
                p = add_dele(merged_data,querylist,querydict,datas_dict,interval_new)#合并区间后，中间查询数组中删除原来的key，添加新的key
                interval_index = p[0] #querylist中受影响的那段的下标
                two_datas_dict = p[1] #受影响的那段querylist   被合并的 data 对象及其在 querylist 中对应的原始索引
                ...
                for i in range(interval_index[0],interval_index[1]):
                    ...
                    if initial_querylist[i].number + 1 == initial_querylist[i+1].number:
                        ...
                        #获取 NOI 位置的过程为：将 NOI 的两个 Intervals取交集，得到包含该 NOI 的所有区间，其中最短区间及位置，即为该 NOI 的匹配区间和位置。
                        list_index_pre = list(initial_querylist[i].indexs & initial_querylist[i+1].indexs)#将相邻区间的端点所属的data集合，取交集
                        #获取到最小区间的经纬度
                        tmp2 = get_lon_lat(list_index_pre,initial_datas_dict)# tmp2 = [datas[0].lng, datas[0].lat,datas[0].locals] 最小区间经纬度
                        ...
                        #获取原始与聚合后位置，如果距离大于1000km，将新区间个数+1
                        #验证 AL 内所有 NOI 的位置，即获取 NOI 的原始位置与聚合后位置，并计算距离，不在 AR 范围内的 NOI 与原始位置组成 CI
                        if distance(location_pre_last[0][0],location_pre_last[0][1],location_pre_last[1][0],location_pre_last[1][1]) > range1:
                            ...
                            data_new = data(str(new_iprefix),location_pre_last[0][0],location_pre_last[0][1],initial_querylist[i].number,initial_querylist[i+1].number)
                            new_iprefix += 1
                            data_new.import_locals(location_pre_last[0][2])
                            #i和i+1是ci的区间端点在querylist的下标
                ...
                if len(new_data_list) != 0 or flag==False:#如果没有受到影响的区间则，直接合并就好
                    #如果受影响的子区间的个数大于1，则将那些经纬度相同的受影响的子区间尝试合并 
                    if len(new_data_list) >= 2:
                        ...
                        while len1>0:
                            len1-=1
                            #new_data_list[i][2] == new_data_list[i+1][1]表示ci和ci之间是连续的
                    ...
                    if len(new_data_list) <= 1 and flag==True : #判定几个受影响区间可以合并并插入
                        ...
                        #处理原来的datas和diss   更新diss的距离，删除合并后的data
                        ...
                        for i in range(len(new_data_list)):
                            ...
                            #更新新插入的ci的diss
                            t = diss[new_index-1]
                            if new_index >0: #不是放在第一个 更新前一个前缀区间跟ci的距离
                            ...
                            if new_index +1 < len(datas): #不是放在最后一个  更新ci和后一个前缀区间的距离
